...
/**
 * INTERFAZ PARA RETO DE COMENTARIOS
 */
interface Comment {
  postId: number;
  id: number;
...
const fetchCommentsByPost = async (postId: number): Promise<void> => {
  ...
  if (IS_DEBUG_MODE) {
    console.log(`%c [RETO] Buscando comentarios del post ID: ${postId}`, "color: purple; font-weight: bold;");
  }
  ...
  try {
    ...
    console.error("Fallo al obtener comentarios:", error);
  }
};
 ...
 */
const SUPABASE_URL: string = "https://lwlqaklbpzmynhpzzyda.supabase.co";
const SUPABASE_KEY: string = "sb_publishable_FFvCP2wKcZfL8alFVlVkSg_F2Gpm8IU";
...
const postAutos = async (): Promise<void> => {
  
  // Realizamos la consulta: 
  //1. .from('autos') -> Selecciona la tabla de tu imagen.
 // 2. .select('*')   -> Pide todas las columnas.
