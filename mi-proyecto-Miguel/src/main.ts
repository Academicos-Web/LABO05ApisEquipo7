...
const fetchCommentsByPost = async (postId: number): Promise<void> => {
  ...
  console.log(`%c [RETO] Buscando comentarios del post con ID: ${postId}...`, "color: magenta; font-weight: bold;");
  ...
  try {
    ...
    // Tip: Usa data.length.
    console.log(`✅ Se encontraron ${data.length} comentarios para el post ${postId}:`);
    ...
    data.forEach((comment: Comment, index: number) => {
      console.log(`${index + 1}. Email: ${comment.email}`);
      console.log(`   Nombre: ${comment.name}`);
      console.log(`   Comentario: ${comment.body.substring(0, 80)}...`);
      console.log("-".repeat(50));
    });
